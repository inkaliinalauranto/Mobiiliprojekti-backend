...
async def get_total_consumption_statistics_daily_for_a_week(dw: DW, date: str):
    ...
    """
    _query = text(
        "SELECT DATE(TIMESTAMP(CONCAT_WS('-', d.year, d.month, d.day))) as day, sum(value) AS total_kwh FROM total_consumptions_fact f JOIN dates_dim d ON d.date_key = f.date_key WHERE DATE(TIMESTAMP(CONCAT_WS('-', d.year, d.month, d.day))) BETWEEN DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND :date GROUP BY d.day ORDER BY day;")
    rows = dw.execute(_query, {"date": date})
    data = rows.mappings().all()
